...
通常の作業は Codex モデルが担い、Fable 5.1 は難所の相談に使う。メインは Astra を含む現在選択中の Codex モデルを継承する。切り出せる実装や大きな調査は下表の担当へ渡し、小さな変更は直接実装する。
...
委譲先には目的・対象・制約・完了条件、作業ルート、必要な参照パスを渡す。資料は担当が必要箇所だけ読み、根拠の位置と要点を返す。メインは全文を読み直さず、矛盾や重大なリスクがある箇所を掘り下げる。実装とテスト作成は排他的な変更対象を持ち、メインが差分で境界を確認する。
...
2026-09-06: メインのコンテキスト消費と委譲を含む総作業量を抑える。Astra の判断力を統合へ使い、切り出せる作業は適切な軽量モデルへ渡すためである。すべてをメインが再調査する構成と、局所的な変更まで多段委譲する構成は採用しない。検証結果は適用対象が一致する限り再利用する。
